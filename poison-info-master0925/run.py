...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        grid.addLayout(left_grid, 1, 0)
        grid.addLayout(right_grid, 1, 1)
        
        self.setWindowTitle('POSION SEARCH')
        self.setLayout(grid)
        self.center()
        self.show()
    ...
    def search_btn_click_event(self):
        ...
        if searched_type == 'CAS':
            try:
                ...
                QMessageBox.about(self, "Error", "Please enter the correct CAS.")
                return
        elif searched_type == 'IUPAC':
            try:
                searched_cid = df[df['IUPAC']==searched_input]['cid'].item()
            except:
                QMessageBox.about(self, "Error", "Please enter the correct IUPAC_NAME.")
                return
        elif searched_type == 'SMILES':
            try:
                searched_cid = df[df['SMILES']==searched_input]['cid'].item()
            except:
                QMessageBox.about(self, "Error", "Please enter the correct SMILES.")
                return
        else:
    ...
    def file_btn_click_event(self, clicked_btn):
        ...
        try:
            self.msms_db[smiles]
            fig, ax = utils.plot_mass(self.mz_array, self.intensity_array, save=True)
